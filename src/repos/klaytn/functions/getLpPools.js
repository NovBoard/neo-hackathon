...
import { prepareLpPools } from "./prepareLpPools.js";

// KAS 접속 정보 설정
const accessKeyId = "KASK2U8TSYABXYY6NZYAEN0F" // KAS 콘솔에서 발급받은 access key id
const secretAccessKey = process.env.REACT_APP_KAS_SECRET_KEY // KAS 콘솔에서 발급받은 secret access key
...
async function getLpPools(tokens, walletAddress) {
    // balanceOf 함수의 데이터를 준비합니다.
    let balanceOfData = await prepareLpPools(tokens, walletAddress)
    ...
    for (let result of results) {
        for (let callResult of result.returnData) {
            balances.push(caver.abi.decodeParameter('uint256', callResult));
        }
    }
