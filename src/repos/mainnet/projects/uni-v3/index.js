...
import { getTokenInfo } from "../../functions/getTokenInfo.js";
import { getUniswapV3AllPools } from "./getUniswapV3AllPools.js";
...
const fetchUniswapV3Pools = async (walletAddress) => {
    ...
    // console.time("Uniswap V3: multicall")
    ...
    // console.timeEnd("Uniswap V3: multicall")
    ...
    const slot0Result = slot0ReturnData.map(data => {
        try{
    ...
    const collectResult = collectReturnData.map(data => {
        try{
            return web3.eth.abi.decodeParameter("(uint256, uint256)", data);
        } catch(e){
            return [0n, 0n];
        }
    });
